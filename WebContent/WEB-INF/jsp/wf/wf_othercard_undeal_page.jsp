...
<script type="text/javascript">
	...
	pageinfo.pageUrl = "/<%=WebConfig.getProperty("web.path")%>/wf/bindothercard/undeal/page.json/all";
	pageinfo.styleJson = "{'pStyle':{'tableStyle':{'tableCol':["
        ...
        +"{'colName':'wfVdate','colText':'有效日期','colWidth':'100px;','linkStat':'0','orderStat':'0'},"
        +"{'colName':'wfVcode','colText':'安全码','colWidth':'100px;','linkStat':'0','orderStat':'0'},"
	...
	pageinfo.crumbString = "流程管理,超级认证";
</script>
...
function fnEditItem(itemid){
	fnCreateEditDlg(
		"self","dialog-form","超级认证审核",
		"/"+basepath+"/wf/bindothercard/undeal/edit/normal/"+itemid,
		"/"+basepath+"/wf/bindothercard/undeal/update.json/normal/"+itemid,
		"保存成功","保存失败"
	);
}
...
function submitEditCheck(obj,itemid){
	var saveurl = "/"+basepath+"/wf/bindothercard/undeal/update.json/normal/"+itemid;
	var tres = $('#dlgframe').contents().find("input[name=result]:checked").val();
	var iyj = $('#dlgframe').contents().find("textarea[name=spyj]").val();
	if(tres=="0"&&iyj==""){
		alert("请填写审批意见！");return false;
	}
	var ires = "通过";if(tres=="0"){ires="驳回";}
	if(!confirm("您确定要【"+ires+"】该申请吗？")){
		return false;
	}
	$.ajax({
		cache: false,
