...
<script type="text/javascript">
	...
	pageinfo.pageUrl = "/<%=WebConfig.getProperty("web.path")%>/channel/mcc/page.json/all";
	pageinfo.styleJson = "{'pStyle':{'tableStyle':{'tableCol':["
	                   +"{'colName':'cfeeChannelid','colText':'渠道编号','colWidth':'100px;','linkStat':'0','orderStat':'0'},"
	                   +"{'colName':'cfeeId','colText':'费率编号','colWidth':'80px;','linkStat':'0','orderStat':'0'},"
	                   +"{'colName':'cfeeName','colText':'费率名称','colWidth':'150px;','linkStat':'0','orderStat':'0'},"
	                   +"{'colName':'cfeeIstop','colText':'是否封顶','colWidth':'80px','linkStat':'0','orderStat':'0'},"
	                   +"{'colName':'cfeePtop','colText':'封顶金额','colWidth':'80px','linkStat':'0','orderStat':'0'},"
	                   +"{'colName':'cfeePmcc','colText':'费率金额','colWidth':'80px','linkStat':'0','orderStat':'0'},"
	                   +"{'colName':'cfeeSpratio','colText':'结算比例','colWidth':'80px','linkStat':'0','orderStat':'0'},"
	                   +"{'colName':'cfeeStat','colText':'状态','colWidth':'80px','linkStat':'0','orderStat':'0'},"
...
function fnCreateItem(){
	fnCreateEditDlg(
			"self","dialog-form","添加渠道商户",
			"/"+basepath+"/channel/mch/add/normal/"+treeinfo.curnode,
			"/"+basepath+"/channel/mch/save.json/normal",
			"保存成功！","提交失败！"
		);
}
function fnEditItem(itemid){
	fnCreateEditDlg(
		"self","dialog-form","添加渠道商户",
		"/"+basepath+"/channel/mch/edit/normal/"+itemid,
		"/"+basepath+"/channle/mch/update.json/normal/"+itemid,
		"保存成功","保存失败"
	);
}
function fnShowItem(itemid){
	fnCreateShowDlg(
		"self","dialog-form","查看渠道商户信息",
		"/"+basepath+"/channel/mch/show/normal/"+itemid
	);
}
function fnDeleteItem(itemid){
	if(confirm("要删除该条数据吗?")){
		$.ajax({
        	type : "GET", 
 			url : "/"+basepath+"/channel/mch/del.json/normal/"+itemid,
			data : "",
			dataType: "text", 
			...
			success : function(msg) {
				alert("删除数据成功！");
				fnRefreshTable("/"+basepath+"/channel/mch/page.json/normal/1/15");
			}
		});
	}
}
...
function submitEditCheck(obj,itemid){
	var saveurl = "/"+basepath+"/channel/mch/update.json/normal/"+itemid;
