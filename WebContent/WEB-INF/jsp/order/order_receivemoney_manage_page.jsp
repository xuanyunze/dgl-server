...
<script type="text/javascript">
	...
	pageinfo.pageUrl = "/<%=WebConfig.getProperty("web.path")%>/order/receivemoney/${ordertype}/page.json/manage";
	pageinfo.styleJson = "{'pStyle':{'tableStyle':{'tableCol':["
        ...
        +"{'colName':'odValue','colText':'订单金额','colWidth':'80px;','linkStat':'0','orderStat':'0'},"
        ...
        +"{'colName':'tranAccno','colText':'交易卡号','colWidth':'160px;','linkStat':'0','orderStat':'0'},"
        +"{'colName':'odStat','colText':'订单状态','colWidth':'80px;','linkStat':'0','orderStat':'0'},"
	    +"{'colName':'odId','colText':'交易编号','colWidth':'150px;','linkStat':'0','orderStat':'0'},"
	    +"{'colName':'tranChlid','colText':'渠道编号','colWidth':'60px;','linkStat':'0','orderStat':'0'},"
	    +"{'colName':'tranMchname','colText':'渠道商户','colWidth':'200px;','linkStat':'0','orderStat':'0'},"
	    +"{'colName':'tranFeerate','colText':'渠道费率','colWidth':'80px;','linkStat':'0','orderStat':'0'},"
	    +"{'colName':'tranIstop','colText':'是否封顶','colWidth':'80px;','linkStat':'0','orderStat':'0'},"
	    +"{'colName':'tranTopvalue','colText':'封顶值','colWidth':'60px;','linkStat':'0','orderStat':'0'},"
	    +"{'colName':'tranBenefit','colText':'渠道手续费','colWidth':'80px;','linkStat':'0','orderStat':'0'},"
	    +"{'colName':'tranStat','colText':'交易状态','colWidth':'80px;','linkStat':'0','orderStat':'0'}"
        +"]}}}";
...
function setAvator(objjson){
	for(var i=0;i<fnGetJsonLength(objjson.mObject.list);i++){
		...
		if(bobj.odType=='9'){bobj.odType='普通收款';}else if(bobj.odType=='11'){bobj.odType='快速收款S0';}
		...
		else if(bobj.odStat=='9'){bobj.odStat='清算失败';}else{bobj.odStat='交易失败['+bobj.odStat+']';}
		if(bobj.odIstop=='0'){bobj.odIstop='否';}else{bobj.odIstop='是';}
		if(bobj.tranIstop=='0'){bobj.tranIstop='否';}else{bobj.tranIstop='是';}
		if(bobj.tranStat==1){bobj.tranStat='交易成功';}else{bobj.tranStat='交易失败['+bobj.tranCode+']';}
	}
	return objjson;
}
...
function fnShowItem(itemid){
	fnCreateShowDlg(
		"self","dialog-form","查看交易信息",
		"/"+basepath+"/order/receivemoney/${ordertype}/show/manage/"+itemid
	);
}
