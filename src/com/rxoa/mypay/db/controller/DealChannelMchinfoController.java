...
/**
 * deal_channel_mchinfo Controller控制器类
 * Tue Apr 05 11:23:27 CST 2016 baiyu
 */ 

@Controller
@RequestMapping("/dbmgr/dealchannelmchinfo")
public class DealChannelMchinfoController {
	private String basedir = "dbmgr";
	@Resource(name="dealChannelMchinfoService")
	private IDealChannelMchinfoService mService;
	...
	@SessionPermission
	public void execSaveJson(HttpServletRequest req,@ModelAttribute("mObject") DealChannelMchinfo mObject,@PathVariable String usetype,PrintWriter printWriter){
		ModelAndView mv = new ModelAndView();
	...
	@SessionPermission
	public void execUpdateJson(HttpServletRequest req,@ModelAttribute("mObject") DealChannelMchinfo mObject,@PathVariable String unid,@PathVariable String usetype,PrintWriter printWriter){
		ModelAndView mv = new ModelAndView();
	...
	public void getPage(ModelAndView mv,PageWeb web,String curpage,String psize,PrintWriter printWriter,String useType,int resType){
		if(resType==0){
			mv.setViewName(basedir+"/deal_channel_mchinfo_page");
		}else if(resType==1){
		}
	}
	...
	public void getCreate(ModelAndView mv,PrintWriter printWriter,String useType,int resType){
		List<Map<String,String>> varObject = CodeUtil.getPoFields("com.rxoa.mypay.db.po.DealChannelMchinfo");
		mv.addObject("subpath","dealchannelmchinfo");
		mv.addObject("varObject",varObject);
		if(resType==0){
			mv.setViewName(basedir+"/deal_channel_mchinfo_add");
		}else if(resType==1){
			JsonUtil.print(mv.getModel(), printWriter);
		}
	}

	public void doSave(ModelAndView mv,DealChannelMchinfo mObject,PrintWriter printWriter,String useType,int resType){
		((DealChannelMchinfo)mObject).setCreTime(new Date());
	...
	public void getEdit(ModelAndView mv,String unid,PrintWriter printWriter,String useType,int resType){
		DealChannelMchinfo mObject = mService.findOne(unid);
		List<Map<String,String>> varObject = CodeUtil.getPoFields("com.rxoa.mypay.db.po.DealChannelMchinfo");
		mv.addObject("subpath","dealchannelmchinfo");
		mv.addObject("varObject",varObject);
		mv.addObject("mObject", mObject);
		if(resType==0){
			mv.setViewName(basedir+"/deal_channel_mchinfo_edit");
		}else if(resType==1){
			JsonUtil.print(mv.getModel(), printWriter);
		}
	}

	public void doUpdate(ModelAndView mv,DealChannelMchinfo mObject,String unid,PrintWriter printWriter,String useType,int resType){
	...
	public void showDetail(ModelAndView mv,String unid,PrintWriter printWriter,String useType,int resType){
		...
		mv.addObject("mObject", mObject);
		if(resType==0){
			mv.setViewName(basedir+"/deal_channel_mchinfo_show");
		}else if(resType==1){
			JsonUtil.print(mv.getModel(), printWriter);
		}
	}
