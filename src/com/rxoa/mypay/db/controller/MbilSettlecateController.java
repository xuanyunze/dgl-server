...
/**
 * mbil_settlecate Controller控制器类
 * Tue Apr 05 11:23:28 CST 2016 baiyu
 */ 

@Controller
@RequestMapping("/dbmgr/mbilsettlecate")
public class MbilSettlecateController {
	private String basedir = "dbmgr";
	@Resource(name="mbilSettlecateService")
	private IMbilSettlecateService mService;
	...
	@SessionPermission
	public void execSaveJson(HttpServletRequest req,@ModelAttribute("mObject") MbilSettlecate mObject,@PathVariable String usetype,PrintWriter printWriter){
		ModelAndView mv = new ModelAndView();
	...
	@SessionPermission
	public void execUpdateJson(HttpServletRequest req,@ModelAttribute("mObject") MbilSettlecate mObject,@PathVariable String unid,@PathVariable String usetype,PrintWriter printWriter){
		ModelAndView mv = new ModelAndView();
	...
	public void getPage(ModelAndView mv,PageWeb web,String curpage,String psize,PrintWriter printWriter,String useType,int resType){
		if(resType==0){
			mv.setViewName(basedir+"/mbil_settlecate_page");
		}else if(resType==1){
		}
	}
	...
	public void getPageData(ModelAndView mv,PageWeb web,String curpage,String psize,PrintWriter printWriter,String useType,int resType){
		...
		List<Map<String,String>> varObject = CodeUtil.getPoFields("com.rxoa.mypay.db.po.MbilSettlecate");
	...
	public void getCreate(ModelAndView mv,PrintWriter printWriter,String useType,int resType){
		List<Map<String,String>> varObject = CodeUtil.getPoFields("com.rxoa.mypay.db.po.MbilSettlecate");
		mv.addObject("subpath","mbilsettlecate");
		mv.addObject("varObject",varObject);
		if(resType==0){
			mv.setViewName(basedir+"/mbil_settlecate_add");
		}else if(resType==1){
			JsonUtil.print(mv.getModel(), printWriter);
		}
	}

	public void doSave(ModelAndView mv,MbilSettlecate mObject,PrintWriter printWriter,String useType,int resType){
		((MbilSettlecate)mObject).setCreTime(new Date());
	...
	public void getEdit(ModelAndView mv,String unid,PrintWriter printWriter,String useType,int resType){
		MbilSettlecate mObject = mService.findOne(unid);
		List<Map<String,String>> varObject = CodeUtil.getPoFields("com.rxoa.mypay.db.po.MbilSettlecate");
		mv.addObject("subpath","mbilsettlecate");
		mv.addObject("varObject",varObject);
		mv.addObject("mObject", mObject);
		if(resType==0){
			mv.setViewName(basedir+"/mbil_settlecate_edit");
		}else if(resType==1){
			JsonUtil.print(mv.getModel(), printWriter);
		}
	}

	public void doUpdate(ModelAndView mv,MbilSettlecate mObject,String unid,PrintWriter printWriter,String useType,int resType){
	...
	public void showDetail(ModelAndView mv,String unid,PrintWriter printWriter,String useType,int resType){
		...
		mv.addObject("mObject", mObject);
		if(resType==0){
			mv.setViewName(basedir+"/mbil_settlecate_show");
		}else if(resType==1){
			JsonUtil.print(mv.getModel(), printWriter);
		}
	}
