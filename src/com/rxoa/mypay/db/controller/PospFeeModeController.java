...
/**
 * posp_fee_mode Controller控制器类
 * Tue Apr 05 11:23:29 CST 2016 baiyu
 */ 

@Controller
@RequestMapping("/dbmgr/pospfeemode")
public class PospFeeModeController {
	private String basedir = "dbmgr";
	@Resource(name="pospFeeModeService")
	private IPospFeeModeService mService;
	...
	@SessionPermission
	public void execSaveJson(HttpServletRequest req,@ModelAttribute("mObject") PospFeeMode mObject,@PathVariable String usetype,PrintWriter printWriter){
		ModelAndView mv = new ModelAndView();
	...
	@SessionPermission
	public void execUpdateJson(HttpServletRequest req,@ModelAttribute("mObject") PospFeeMode mObject,@PathVariable String unid,@PathVariable String usetype,PrintWriter printWriter){
		ModelAndView mv = new ModelAndView();
	...
	public void getPage(ModelAndView mv,PageWeb web,String curpage,String psize,PrintWriter printWriter,String useType,int resType){
		if(resType==0){
			mv.setViewName(basedir+"/posp_fee_mode_page");
		}else if(resType==1){
		}
	}
	...
	public void getPageData(ModelAndView mv,PageWeb web,String curpage,String psize,PrintWriter printWriter,String useType,int resType){
		...
		List<Map<String,String>> varObject = CodeUtil.getPoFields("com.rxoa.mypay.db.po.PospFeeMode");
	...
	public void getCreate(ModelAndView mv,PrintWriter printWriter,String useType,int resType){
		List<Map<String,String>> varObject = CodeUtil.getPoFields("com.rxoa.mypay.db.po.PospFeeMode");
		mv.addObject("subpath","pospfeemode");
		mv.addObject("varObject",varObject);
		if(resType==0){
			mv.setViewName(basedir+"/posp_fee_mode_add");
		}else if(resType==1){
			JsonUtil.print(mv.getModel(), printWriter);
		}
	}

	public void doSave(ModelAndView mv,PospFeeMode mObject,PrintWriter printWriter,String useType,int resType){
		((PospFeeMode)mObject).setCreTime(new Date());
	...
	public void getEdit(ModelAndView mv,String unid,PrintWriter printWriter,String useType,int resType){
		PospFeeMode mObject = mService.findOne(unid);
		List<Map<String,String>> varObject = CodeUtil.getPoFields("com.rxoa.mypay.db.po.PospFeeMode");
		mv.addObject("subpath","pospfeemode");
		mv.addObject("varObject",varObject);
		mv.addObject("mObject", mObject);
		if(resType==0){
			mv.setViewName(basedir+"/posp_fee_mode_edit");
		}else if(resType==1){
			JsonUtil.print(mv.getModel(), printWriter);
		}
	}

	public void doUpdate(ModelAndView mv,PospFeeMode mObject,String unid,PrintWriter printWriter,String useType,int resType){
	...
	public void showDetail(ModelAndView mv,String unid,PrintWriter printWriter,String useType,int resType){
		...
		mv.addObject("mObject", mObject);
		if(resType==0){
			mv.setViewName(basedir+"/posp_fee_mode_show");
		}else if(resType==1){
			JsonUtil.print(mv.getModel(), printWriter);
		}
	}
