...
/**
 * mbil_feed Controller控制器类
 * Tue Apr 05 11:23:28 CST 2016 baiyu
 */ 

@Controller
@RequestMapping("/dbmgr/mbilfeed")
public class MbilFeedController {
	private String basedir = "dbmgr";
	@Resource(name="mbilFeedService")
	private IMbilFeedService mService;
	...
	@SessionPermission
	public void execSaveJson(HttpServletRequest req,@ModelAttribute("mObject") MbilFeed mObject,@PathVariable String usetype,PrintWriter printWriter){
		ModelAndView mv = new ModelAndView();
	...
	@SessionPermission
	public void execUpdateJson(HttpServletRequest req,@ModelAttribute("mObject") MbilFeed mObject,@PathVariable String unid,@PathVariable String usetype,PrintWriter printWriter){
		ModelAndView mv = new ModelAndView();
	...
	public void getPage(ModelAndView mv,PageWeb web,String curpage,String psize,PrintWriter printWriter,String useType,int resType){
		if(resType==0){
			mv.setViewName(basedir+"/mbil_feed_page");
		}else if(resType==1){
		}
	}
	...
	public void getPageData(ModelAndView mv,PageWeb web,String curpage,String psize,PrintWriter printWriter,String useType,int resType){
		...
		List<Map<String,String>> varObject = CodeUtil.getPoFields("com.rxoa.mypay.db.po.MbilFeed");
	...
	public void getCreate(ModelAndView mv,PrintWriter printWriter,String useType,int resType){
		List<Map<String,String>> varObject = CodeUtil.getPoFields("com.rxoa.mypay.db.po.MbilFeed");
		mv.addObject("subpath","mbilfeed");
		mv.addObject("varObject",varObject);
		if(resType==0){
			mv.setViewName(basedir+"/mbil_feed_add");
		}else if(resType==1){
			JsonUtil.print(mv.getModel(), printWriter);
		}
	}

	public void doSave(ModelAndView mv,MbilFeed mObject,PrintWriter printWriter,String useType,int resType){
		((MbilFeed)mObject).setCreTime(new Date());
	...
	public void getEdit(ModelAndView mv,String unid,PrintWriter printWriter,String useType,int resType){
		MbilFeed mObject = mService.findOne(unid);
		List<Map<String,String>> varObject = CodeUtil.getPoFields("com.rxoa.mypay.db.po.MbilFeed");
		mv.addObject("subpath","mbilfeed");
		mv.addObject("varObject",varObject);
		mv.addObject("mObject", mObject);
		if(resType==0){
			mv.setViewName(basedir+"/mbil_feed_edit");
		}else if(resType==1){
			JsonUtil.print(mv.getModel(), printWriter);
		}
	}

	public void doUpdate(ModelAndView mv,MbilFeed mObject,String unid,PrintWriter printWriter,String useType,int resType){
	...
	public void showDetail(ModelAndView mv,String unid,PrintWriter printWriter,String useType,int resType){
		...
		mv.addObject("mObject", mObject);
		if(resType==0){
			mv.setViewName(basedir+"/mbil_feed_show");
		}else if(resType==1){
			JsonUtil.print(mv.getModel(), printWriter);
		}
	}
