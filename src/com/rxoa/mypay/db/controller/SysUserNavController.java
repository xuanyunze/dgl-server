...
/**
 * sys_user_nav Controller控制器类
 * Tue Apr 05 11:23:29 CST 2016 baiyu
 */ 

@Controller
@RequestMapping("/dbmgr/sysusernav")
public class SysUserNavController {
	private String basedir = "dbmgr";
	@Resource(name="sysUserNavService")
	private ISysUserNavService mService;
	...
	@SessionPermission
	public void execSaveJson(HttpServletRequest req,@ModelAttribute("mObject") SysUserNav mObject,@PathVariable String usetype,PrintWriter printWriter){
		ModelAndView mv = new ModelAndView();
	...
	@SessionPermission
	public void execUpdateJson(HttpServletRequest req,@ModelAttribute("mObject") SysUserNav mObject,@PathVariable String unid,@PathVariable String usetype,PrintWriter printWriter){
		ModelAndView mv = new ModelAndView();
	...
	public void getPage(ModelAndView mv,PageWeb web,String curpage,String psize,PrintWriter printWriter,String useType,int resType){
		if(resType==0){
			mv.setViewName(basedir+"/sys_user_nav_page");
		}else if(resType==1){
		}
	}
	...
	public void getPageData(ModelAndView mv,PageWeb web,String curpage,String psize,PrintWriter printWriter,String useType,int resType){
		...
		List<Map<String,String>> varObject = CodeUtil.getPoFields("com.rxoa.mypay.db.po.SysUserNav");
	...
	public void getCreate(ModelAndView mv,PrintWriter printWriter,String useType,int resType){
		List<Map<String,String>> varObject = CodeUtil.getPoFields("com.rxoa.mypay.db.po.SysUserNav");
		mv.addObject("subpath","sysusernav");
		mv.addObject("varObject",varObject);
		if(resType==0){
			mv.setViewName(basedir+"/sys_user_nav_add");
		}else if(resType==1){
			JsonUtil.print(mv.getModel(), printWriter);
		}
	}

	public void doSave(ModelAndView mv,SysUserNav mObject,PrintWriter printWriter,String useType,int resType){
		((SysUserNav)mObject).setCreTime(new Date());
	...
	public void getEdit(ModelAndView mv,String unid,PrintWriter printWriter,String useType,int resType){
		SysUserNav mObject = mService.findOne(unid);
		List<Map<String,String>> varObject = CodeUtil.getPoFields("com.rxoa.mypay.db.po.SysUserNav");
		mv.addObject("subpath","sysusernav");
		mv.addObject("varObject",varObject);
		mv.addObject("mObject", mObject);
		if(resType==0){
			mv.setViewName(basedir+"/sys_user_nav_edit");
		}else if(resType==1){
			JsonUtil.print(mv.getModel(), printWriter);
		}
	}

	public void doUpdate(ModelAndView mv,SysUserNav mObject,String unid,PrintWriter printWriter,String useType,int resType){
	...
	public void showDetail(ModelAndView mv,String unid,PrintWriter printWriter,String useType,int resType){
		...
		mv.addObject("mObject", mObject);
		if(resType==0){
			mv.setViewName(basedir+"/sys_user_nav_show");
		}else if(resType==1){
			JsonUtil.print(mv.getModel(), printWriter);
		}
	}
